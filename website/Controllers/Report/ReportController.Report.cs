using Milton.Website.Models.Report;
using System;
...
namespace Milton.Website.Controllers.Report
{
    ...
    {
        public ActionResult Generate(string fromDate, string toDate)
        {
            DateTime startDate;
            DateTime endDate;
            ...
            var result = _patientService.Report(startDate, endDate);

            ReportViewModel model = new ReportViewModel();
            model.Lodge = new List<ReportLodgeViewModel>();
            model.Lodge.AddRange(result.Select(x => new ReportLodgeViewModel()
            {
                Accommodation = x.DaysAccommodation,
                IdNumber = x.IdNumber,
                Name = x.Name,
                Surname = x.Surname,
                Total = CalculateLodge(x)

            }));
            model.Transport = new List<ReportTransportViewModel>();
            model.Transport.AddRange(result.Select(x => new ReportTransportViewModel()
            {
                Transport = (x.NormalTransport + x.AdmittedTransport + x.DischargedTransport),
                IdNumber = x.IdNumber,
                Name = x.Name,
                Surname = x.Surname,
                Total = CalculateTransport(x)
